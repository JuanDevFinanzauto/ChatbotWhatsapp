...
def validar_satisfaccion(state: GraphState):
    ...
    return "validar_respuesta_satisfaccion"
    ...
    while current_node_name:
        ...
        if next_node_name:
            if next_node_name in graph.edges.get(current_node_name, []):
                print(f"Transición: {current_node_name} -> {next_node_name}")  # Depuración
                current_node_name = next_node_name
            else:
                print(f"Error: Transición de '{current_node_name}' a '{next_node_name}' no está definida.")
                break
        else:
...
def run_graph(user_id):
    ...
    execute_graph(graph, "validado", state)
    ...
    while user_data.conversation_active:
        try:
            ...
            if state.user_data.awaiting_response:
                # Ejecutar el siguiente nodo basado en el flujo
                # Las funciones de nodos manejan las transiciones automáticamente
                # Por lo tanto, llamamos a execute_graph con el próximo nodo retornado por el último estado
                pass  # Las transiciones ya están manejadas en execute_graph
            else:
